...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        {
            let res = self.chunks.remove(idx);
            return Ok(res);
        }
        Err("Doesn't mattch".into())
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let bytes = self.chunks.iter().flat_map(|c| c.as_bytes());
        self.header.iter().cloned().chain(bytes).collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        if buffer != Png::STANDARD_HEADER {
            return Err("signature wrong err".into());
        }
        let mut i: usize = 8;
        while i < bytes.len() {
            ...
            let idx = i + length + 4 * 3;
            let result = Chunk::try_from(&bytes[i..idx]);
            if let Ok(res) = result {
                chunks.push(res);
            } else {
                return Err("chunk fill error".into());
            }
            // i doesn't known length's len
            reader.consume(idx - i - 4);
            i = idx;
        }
        Ok(Png {
            header: buffer,
